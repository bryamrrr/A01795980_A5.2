...
It takes two JSON files as input: one with product prices and another with sales records.
The output is printed to the console and saved in a file named 'SalesResults.txt'.
...
# The first file will contain information in a JSON format about a catalogue of prices
# of products. The second file will contain a record for all sales in a company.
...
# execution and calculus of the data. This number shall be included in the results
...
def convert_catalogue_to_dict(price_catalogue):
    ...
    Args:
        price_catalogue (list): List of product dictionaries with 'product' and 'price' keys.
...
def compute_total_sales(price_catalogue, sales_record):
    """
    Compute the total sales cost based on the provided price catalogue and sales record.
    
    Args:
        price_catalogue (list): List of products and their prices.
        sales_record (list): List of sales transactions, each with product names and quantities.
    ...
    for sale in sales_record:
        ...
        if not isinstance(quantity, (int, float)) or quantity < 0:
            print(f"Warning: Invalid quantity for product '{product}'. Skipping.")
            continue
...
def main():
    ...
    if not isinstance(price_catalogue, list) or not isinstance(sales_record, list):
